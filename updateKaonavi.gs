/**
 * 変更申請_SmartHRからOBICへの社員情報連携登録処理
 * 
 * 更新対象社員選択_スプレッドシート上で選択された社員の社員番号と、
 * 履歴データ[更新用]_スプレッドシートに記載されている社員番号を照合し、
  ...
  try {
    ...
    if(lastRow > headerLine) {
      ...
      if(selectedEmployeeList.length > 0) {
        ...
        if(employeeCodeList.length > 0) {
          ...
          for (var l = 0; l < employeesData.length; l++) {
            ...
            if (employeesData[l]['address'] && employeesData[l]['address']['building']) {
              var building = employeesData[l]['address']['building'];
            } else {
              var building = null;
            }
            if (employeesData[l]['address'] && employeesData[l]['address']['country_number']) {
              var countryNumber = employeesData[l]['address']['country_number'];
            } else {
            ...
            if (employeesData[l]['emergency_address'] && employeesData[l]['emergency_address']['building']) {
              var emergencyAddressBuilding = employeesData[l]['emergency_address']['building'];
            } else {
              var emergencyAddressBuilding = null;
            }
            ...
            for (var efl = 0; efl < employeeFamilyData.length; efl++) {
              ...
              if (employeeFamilyData[efl]['live_together_type'] === "living_together") {
                ...
                if (employeeFamilyData[efl]['address'] && employeeFamilyData[efl]['address']['building']) {
                  familyData[efl+1]['familyBuilding'] = employeeFamilyData[efl]['address']['building'];
                } else {
                  familyData[efl+1]['familyBuilding'] = null;
                }
              }
        ...
        const basicInfoResult = requestApi(basicInfoList, "patch");
      ...
      　log(work, 'e');
        ...
        SpreadsheetApp.getUi().alert("カオナビへの社員情報更新が完了しました。");
        }
      } else {
    ...
    log(e.message, "error");
    log(work + "[エラーログ]", "e");
    SpreadsheetApp.getUi().alert("カオナビへの社員情報更新に失敗しました。");
  }
}
...
/**
 * カオナビAPIへ値をリクエスト送信し、更新する社員の各種情報を登録する
 * 
 * array updateData (更新データ)
 * string methodType (patch:各種シートへの情報登録)
 * integer sheetId カオナビ_シートID
 */
function requestApi(updateData, methodType, sheetId = null) {
  // カオナビAPI_トークン取得
