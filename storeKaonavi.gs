...
function createMember() {
  ...
  // 開始ログ
  commonFunction.log('入社カオナビ連携登録', 's');
  ...
  try{
    for(var l = 0; l < memberData.length; l++) {
      let data = {
        ...
        "name" : memberData[l].last_name + " " + memberData[l].first_name,
        "name_kana":memberData[l].last_name_yomi + " " + memberData[l].first_name_yomi
      }
      dataList.push(data);
    }
    ...
    // 終了ログ
  　commonFunction.log('入社カオナビ連携登録', 'e');
